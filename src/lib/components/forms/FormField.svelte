<script lang="ts">
    ...
    let { value = $bindable(), name, errorMessage = "Invalid!", type = "text" } = $props();
</script>

<Label for="field" class="pb-1 pt-1">{name}</Label>
<Input type={type} class={`w-100% border-2 ${(value.validState != ValidState.Valid) ? "border-red-500" : ""}`} bind:value={value.value} id="field" onblur={() => value.validateThenSet() } />
